...
from illust_v8 import ILLUST_V8 as ILLUST_COMMENTARY
...
for w in WEEKS:
    ...
    # spec 청소: stock illustration "(블록 N 본문 삽화)" 제거 (정본 v9)
    ...
    for bi, block in enumerate(spec.get('blocks', [])):
        ...
        # 기존 main_scripture verbatim 빈 채면 NWT fetch
        ms = comm.get('main_scripture')
        if ms and isinstance(ms, dict) and not ms.get('verbatim'):
            try:
                ...
                print(f'    ⚠️ fetch fail {ms.get("ref")}: {e}')
            ...
            if isinstance(ks, list):
                for item in ks:
                    if isinstance(item, dict) and ms_ref in item.get('ref', ''):
                        ...
                        break
            # NWT 직접 fetch — key_scripture 에 없을 때
            if not ms_verb:
                try:
                    from scrape_wt import fetch_nwt_verse, _parse_range_ref
                    ...
                    print(f'    ⚠️ NWT fetch fail {ms_ref}: {e}')
            comm['main_scripture'] = {'ref': ms_ref, 'verbatim': ms_verb, 'depth_explanation': ''}
            main_n += 1

        # 이미지 적용 — caption "(N항 참조)" + block.numbers 양쪽 매칭 (정본 v9)
        if IMAGE_PATHS:
            seq = block.get('sequence', [])
            ...
            for ill in ill_items:
                ...
                target_n = None
                if cm:
                    target_n = int(cm.group(1))
                else:
                    # 2. block.numbers 매칭
                    bn = str(block.get('numbers', ''))
                    bm = re.match(r'(\d+)', bn)
                    if bm:
                        target_n = int(bm.group(1))
                if target_n is not None:
                    paths = IMAGE_PATHS.get(target_n)
                    if paths:
                        ill['image_path'] = paths[0]
                        img_n += 1

        # 삽화 description + commentary 주입 (illustration-finder v8)
            ...
            for it in seq:
                if isinstance(it, dict) and it.get('type') == 'illustration':
                    ...
                    if cap.startswith('(블록 '):
                        continue
                    # description 자동 base 위에 덮어쓰기 (풍부한 [Y] 마커 버전)
    ...
    # 매핑 안 된 image 추가 로직 폐기 — stock caption B0 박힘 차단 (정본 v9)
    # 자동 base 의 의미있는 illustration item 에 image_path 만 박는 것이 정답
    ...
    print(f"  ✓ INT:{int_n} narr:{narr_n} main:{main_n} verb:{verb_n} img:{img_n} recap:{recap_n}")

    from build_watchtower import build_watchtower
    OUT = f"{BASE}/{folder}/파수대 사회_{ymd}_canon_v9_.docx"
    try:
