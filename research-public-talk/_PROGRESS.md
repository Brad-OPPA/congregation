...
- `S-34_split/_archive_old45/`: 구판 116개 격리.
- 참고 파일:
...
### ⏳ 남은 작업 (선택)
1. `_archive_old45/` 최종 삭제 여부 확정 (현재는 안전하게 격리만).
2. `C:\Users\yoone\tmp_*.py` 임시 스크립트 정리.
3. `S-34_titles.json` (구판 색인) 을 계속 보관할지 판단 (신판 제목이 다른 것이 확인됐으므로 참고용으로만 유지 권장).
