...
brief = get_briefing_for_team("mid-talk")
augmented = prepend_to_prompt(original_prompt, brief)
Agent(subagent_type="treasures-talk-planner", prompt=augmented, ...)
```

또는 CLI:
```bash
python3 _automation/team_briefings.py mid-talk
```
...
```bash
python3 _automation/run_dedup_for_slot.py mid-talk10 <빌드된 docx 경로>
```
