...
# cbs — 회중 성서 연구 사회 30분 (단일 주차, 4단 방어)
...
이 스킬의 모든 에이전트 호출은 `.claude/shared/multi-layer-defense.md` 의 4단 방어 프로토콜에 따른다. 실행 전 반드시 Read.
...
>
> ⚠ 4단 방어 프로토콜 준수: `.claude/shared/multi-layer-defense.md` 를 먼저 Read 해서 당신의 역할 단계 확인 후 작업.

## 📖 저작권 정책
jw.org·wol.jw.org 공개 자료로 저작권 고려 없음 (2026-04-22). 장문 verbatim 허용. 출처 URL 필수.

## 저장 위치
베이스: `C:\Users\yoone\Dropbox\02.WatchTower\01.▣ 수원 연무 회중\01.주중집회\05.회중 성서 연구\YYMMDD_M월 D-D일\`
...
```
Agent(cbs-planner)
  프롬프트: "{YYMMDD} 주차 '회중 성서 연구 (30분)' 기획 (4단 방어 ① 단계).
  ...
  ⚠ 저장: `research-plan/cbs/{YYMMDD}/` outline.md + meta.yaml.
  ...
  ⭐⭐ 추가 필수 키 — instructions_to_subresearchers (4단 방어 ① 단계):
...
Agent(cbs-planner)  [재검수 모드]
  프롬프트: "당신이 지시서를 내린 서브 산출물을 재검수 (4단 방어 ③).
