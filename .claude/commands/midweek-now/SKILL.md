...
**진행 추적 도구** — `_automation/orchestrator.py` 사용 의무:

```bash
# 1) 시작 시 run_id 받기
RID=$(python3 _automation/orchestrator.py init --scope midweek-now)
export ORCHESTRATOR_RUN_ID=$RID

# 2) 그룹 A 9개 Skill 호출 — 각 직전·직후 log
python3 _automation/orchestrator.py log-start mid-talk10
# Skill(mid-talk10) 호출 후
python3 _automation/orchestrator.py log-end mid-talk10 success
# ... 9개 슬롯 모두 동일 ...

# 3) 그룹 A 완료 점검 (멈춤 슬롯 카톡 알림)
python3 _automation/orchestrator.py check --notify

# 4) 그룹 B (chair) — 마찬가지로 log
python3 _automation/orchestrator.py log-start chair
# Skill(chair) 호출 후
python3 _automation/orchestrator.py log-end chair success
