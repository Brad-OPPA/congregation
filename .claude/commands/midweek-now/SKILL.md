...
각 파트 스킬을 인자 `now` 로 순차 호출. 파트 내부에서 이미 주차 UX·WOL-first·할루시네이션 금지·감수 게이트·docx 렌더까지 다 수행하므로 이 스킬은 **얇은 오케스트레이터**.

순서 (WOL 프로그램 순):
1. Skill(mid-talk10, args="now")
2. Skill(dig-treasures, args="now")
3. Skill(mid-student1, args="now")
4. Skill(mid-student2, args="now")
5. Skill(mid-student3, args="now")
6. Skill(mid-student4, args="now")   # 내부에서 슬롯 없으면 자동 skip
7. Skill(mid-talk5, args="now")
8. Skill(living-part, args="now")    # 파트 2개 이상이면 원준님 선택
9. Skill(cbs, args="now")
10. Skill(chair, args="now")          # 마지막 — 다른 파트 다 끝난 후 통합 사회자 대본 (chair 본문에서 8가지 슬롯 대화형 입력)

각 단계 완료 후 결과 경로를 이 스킬 진행 상황에 누적.
