---
name: midweek-next2
description: 2주 뒤 목요일 주중집회 전체 자료를 일괄 생성 (회중의 필요 제외). 11개 파트 스킬을 인자 `next2` 로 순차 호출 — mid-talk10, dig-treasures, mid-student1~4, mid-talk5, living-part, cbs, chair (사회자 전체 대본). 각 파트 독립 docx/PDF. 회중의 필요는 별도로 `/local-needs next2` 실행. 트리거 "/midweek-next2", "2주 뒤 주중집회 만들어 줘".
---
...
# midweek-next2 — 2주 뒤 주중집회 전체 (회중의 필요 제외)
...
## 이 스킬의 범위
- **2주 뒤 목요일 집회** 자료 전체 일괄 생성
- 포함 파트 11개 — `/midweek-now` 와 동일 (주차만 다름)
- **제외 파트**: 회중의 필요 — `/local-needs next2` 별도

## 실행 단계

### 0. 주차 확인
오늘 기준 2주 뒤 목요일 YYMMDD 계산 후 원준님 확정.

### 1. 파트 스킬 순차 호출 (인자 `next2`)

1. Skill(mid-talk10, args="next2")
2. Skill(dig-treasures, args="next2")
3. Skill(mid-student1, args="next2")
4. Skill(mid-student2, args="next2")
5. Skill(mid-student3, args="next2")
6. Skill(mid-student4, args="next2")
7. Skill(mid-talk5, args="next2")
8. Skill(living-part, args="next2")
9. Skill(cbs, args="next2")
10. Skill(chair, args="next2")          # 마지막 — 다른 파트 다 끝난 후 통합 사회자 대본 (chair 본문에서 8가지 슬롯 대화형 입력)
...
```
midweek-next2 ({YYMMDD-MMDD}) 산출물 현황:
| 파트 | 상태 |
|---|---|
| 10분 연설 | ✅ 있음 (ver2) / ❌ 없음 |
| 영적 보물 찾기 | ... |
...
- **신규 빌드 결정** → `Skill(단편, args="next2")` 호출 (단편은 묶음 컨텍스트 안에서 정상 진행)
- **ver_up 결정** → `Skill(단편, args="next2 --from-batch=ver_up")` 형태로 컨텍스트 전달, 단편은 `_verN_` 자동 부여
