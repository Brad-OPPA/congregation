# 6단 방어 프로토콜 (Multi-Layer Defense) — 기존 "6단 방어(v2)" 의 확장
...
**원칙**: 원고의 모든 인용·해설·적용이 공식 출판물 근거를 갖도록 **6중 방어**. 에이전트는 **3번 리마인드** (지시서 → 🟢 착수 블록 → 🔴 종료 블록), 산출물은 **4회 검증** (자체 → Planner 1차 → Planner 2차 → 최종 감사 **4종**).

> **⑥ 단계 갱신 (2026-04-29)**: 최종 감사 게이트가 **3종 → 4종 병렬** 로 확장.
> - fact-checker (사실·인용 verbatim)
> - jw-style-checker (금칙어·호칭·8필터)
> - timing-auditor (낭독 시간, ±60→±120초 완화)
> - **quality-monotonic-checker** ← 신규: 직전 주차 대비 품질 단조 증가 (글자·성구·출판·외부 14축) 자동 점검. FAIL 시 자동 재작성 무한 루프 (5회 한도). 정책: `.claude/shared/quality-monotonic-policy.md`.
>
> 우선순위: **quality > timing**. timing FAIL 이라도 quality PASS 면 통과. 반대는 X.
