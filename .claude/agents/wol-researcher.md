...
| **「쁨」** | **`sjj`** | **‘기쁨으로 노래하라’ (왕국의 노래책 N집)** | `/ko/wol/publication/r8/lp-ko/sjj` |
...
| **「쁨」 노래책** | **`sjj`** | **N번 = `1102016800 + N`** | **(예: 12번 = `1102016812`, 58번 = `1102016858`)** |
| (예: 「행누」 13과 이후 docid 점프 가능 — 책 구조에 따라 차이) | — | — | 아래 검증 절차 의무 |

### 노래책 자동 fetch (chair · week-study 호출 시 의무)

주차 인덱스 raw HTML 에서 `노래 N` 패턴 grep 으로 시작·중간·마치는 노래 번호 추출 → `1102016800 + N` docid 로 fetch → **한국어 제목 + 표어 성구 + 1절 가사 (저작권 고려해 1-2 문장만)** 자동 추출. 사회자 대본의 노래 안내 슬롯에 자동 채움.
