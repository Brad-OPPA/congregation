...
6. **사회자용 독립 후보 패키지 사전 추출** (별도 파일 `chair_advice_candidates.md`)
   ...
   - ④ 주의점 후보 2~3개 — 해당 조언과에서 흔한 약점 (학생 script 와 무관)
   - 실전에서 사회자가 실제 연설 듣고 해당되는 1~2개만 선택
   - 학생이 조언과를 잘 못 구현했을 때도 사회자가 "이런 지점이 더 드러났으면" 이라는 피드백에 쓸 수 있어야 함
...
- 🚫 금지:
  ...
  - 조언과 체현 지점에 인라인 주석 · 메타 표기 (예: "[요점 예고]" 같은 표시 금지)
- 필수 포함:
  - §2 체현 지점 3~5개 중 시간 내 수용 가능한 것 **모두** 자연스럽게 구현
  - 서론 요점 예고 + 결론 요점 재언급 (14과 원칙)
  - 봉사·개인 연구에서 실천할 **한 가지** 구체 행동
- 총 분량 목표: N분 → 약 NN자
```
...
chair_advice_candidates:
  compliment_count: 5                   # §9 ③ 후보 개수
  caution_count: 3                      # §9 ④ 후보 개수
  candidates_path: "outline.md#9"
instructions_to_subresearchers:
  scripture-deep: |
    ...
  publication-cross-ref: |
...
- §2 체현 지점 3~5개 (script 힌트 포함)
- §9 사회자 조언 후보와 §2 체현 지점이 **1:1 대응**

## I. 사회자 조언 패키지 — 학생 원고·사회자 원고 동기화
- §9 ③ 칭찬 후보 3~5개 — 조언과 원칙 기준, 학생 연설 문장에서 체현될 구체 지점
- §9 ④ 주의점 후보 2~3개 — 해당 조언과에서 흔한 약점
- `chair-script-builder` 는 meta.yaml `chair_advice_candidates.candidates_path` 를 따라 §9 를 Read 해서 "(연설 후)" 블록 ③④ 에 재활용

## J. student-talk-planner 특화 — 4파일 계약
```
...
```
→ WOL 주차 5분 연설 파트 파싱 → 참조 자료·조언과 본문 획득 → 지시서 발행 → 4파일 저장
