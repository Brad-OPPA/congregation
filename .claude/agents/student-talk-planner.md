...
4. **5개 보조 리서처에게 지시서 발행** (§3 · 6단 방어(v2) ①)
...
# 6단 방어(v2) 프로토콜 ③ — Planner 재검수 (재검수 모드)
...
- 담당자 자격: 남학생만 (S-38 11항)
- 6단 방어(v2): 지시서 발행 완료 / 재검수 (대기 | PASS | NEEDS-RERUN)
- 추가 조사 갭: (bullet)
---
...
## D. 상단 대시보드 필수
outline.md 첫 블록. 6단 방어(v2) 상태 필드 포함.
...
- [ ] `instructions_to_subresearchers` meta 키에 5개 서브 지시서 기록 (6단 방어(v2) ①)
