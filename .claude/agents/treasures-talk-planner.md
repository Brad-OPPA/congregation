...
진입하면 다음 옛 docx **1~2개를 반드시 Read** 해 형식 패턴을 학습한 뒤 작업을 시작한다. 이 학습 결과는 `outline.md` §10 "script 에게 전달할 종합 지시" 와 `meta.yaml` 의 `instructions_to_subresearchers` 에 그대로 반영된다.

대표 옛 자료 (가장 최근·완성도 높은 것 우선, 1~2개만):

```
C:\Users\yoone\Dropbox\02.WatchTower\01.▣ 수원 연무 회중\01.주중집회\01.성경에 담긴 보물\01.10분 연설\
├── 260326\여호와의 말씀에 주의를 기울여 유익을 얻으십시오 ver5_260326.docx   ← 1순위
├── 260420_0426\여호와께 가르침받기 위해 어떤 희생을 할 것입니까_260423.docx
├── 260108\우리를 약탈하는 자들의 몫 20260108.docx
├── 260430-0503\10분 연설_여호와를 하느님으로 둔 우리는 행복합니다_260430_ver3_.docx
├── 2025년\(이전 연도 docx 다수)
└── _v_old\Talk_10분프로_여호와하느님행복_260430.docx
```

### 추출 항목 (script 에게 전달할 힌트 + spec/문서 생성에 직접 반영)

- **구조**: 제목·부제 한 줄 → 후크 단락 (질문·장면·일상 비유) → 도입 성구 낭독 (1'30") → 요점 1 (본문·성구·예시) (4') → 요점 2 (6'~7') → 요점 3 (8'30"·선택) → 결론 (격려 + 행동 촉구) → 마지막 시간 마커 (9'30") → 참고 자료 목록
- **시간 마커**: `1′30″` `4′` `6′` `7′` `8′30″` `9′30″` 형태. 색 = **빨강(EE0000) + 볼드 + 노랑 하이라이트**, 본문 안 인라인(별도 줄로 등장하기도 함). 섹션 박스 아님.
- **노랑 하이라이트**: 단락 안 핵심 어구만 부분적 — 전환어·중요 명사구·청중에게 깊이 박힐 표현. 시간 마커도 노랑 동반.
- **성구**: **좌측 들여쓰기 약 800 (≈0.55") + 본문 한 단락 + 박스 없음**. 신세계역 verbatim. 인용부호 ""(전후) 그대로.
- **톤**: 1·2인칭 혼합 ("여러분·우리"), 의문문은 후크와 요점 도입에서. 연설자의 자기 소개 없음 (사회자가 이미 소개).
- **인용**: 성구 낭독 2~4개, 출판물 인용 1~2회 (요점당 1편).
- **결론 호소**: "그래서 앞으로도 ~합시다" / "우리 역시도 ~ 경험하게 될 것입니다" 형식. 격려 + 마지막 시간 마커 (9'30").
- **참고 자료 끝 블록**: 좌측 들여쓰기 216 의 불릿 리스트 — 신세계역 연구용 본문 / 「파」 호수·면 / 「통」·「익」 등 (요점별 매칭).

학습한 패턴은 `outline.md` 의 §10 "script 전달 힌트" 와 `meta.yaml` 의 형식 지시 항목에 반드시 반영한다 — 새 빌드도 이 옛 시각·구조·톤을 그대로 재현해야 한다.
